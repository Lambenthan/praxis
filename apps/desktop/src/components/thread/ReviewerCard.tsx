  ...
  return (
    <div className="rounded-card border border-border bg-surface shadow-card">
      ...
      >
        ...
        <span className="text-sm font-medium text-text">{t("Reviewer")}</span>
        <span className="text-sm text-muted">
          · {visible.length} {t(visible.length === 1 ? "finding" : "findings")}
          {dismissed.size > 0 && ` · ${dismissed.size} ${t("dismissed")}`}
        </span>
        <ChevronDown
          size={16}
      ...
      {open && (
        <div className="space-y-3 px-4 pb-4">
                ...
                <div className="flex items-start gap-2">
                  ...
                  {(f.tag || f.check) && (
                    <span className="rounded bg-surface-2 px-1.5 py-0.5 text-xs text-muted ring-1 ring-border">
                      {f.tag ?? (f.check ? t(CHECK_TAG[f.check]) : "")}
                    </span>
                  )}
                  ...
                  <button
                    ...
                    aria-label={`${t("Dismiss finding:")} ${f.title}`}
                    title={t("Dismiss this finding")}
          ...
          {visible.length === 0 && block.findings.length > 0 && (
            <p className="text-sm text-muted">{t("All findings dismissed.")}</p>
          )}
