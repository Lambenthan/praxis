...
describe("InteractionPrompt — question", () => {
  ...
  it("always offers a free-text box and submits a typed answer on Enter", async () => {
    const onAnswer = vi.fn();
    // singleQ declares no `custom` flag — the box must still be there.
    render(<InteractionPrompt question={singleQ} onAnswer={onAnswer} onReject={noop} onPermission={noop} />);
    const box = screen.getByPlaceholderText(/type your own answer/i);
    await userEvent.type(box, "use my cleaned panel instead{Enter}");
    expect(onAnswer).toHaveBeenCalledWith("que_1", [["use my cleaned panel instead"]]);
  });
