...
export function NotebookEditor({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!f || f.encoding !== "utf8") throw new Error(t("could not read the notebook"));
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`${t("Could not save:")} ${e instanceof Error ? e.message : String(e)}`);
    }
  }, [path, root]);
    ...
    try {
      ...
      update(cell.index, {
        output: interruptRef.current
          ? t("Interrupted — the kernel was restarted; variables were reset.")
          : `${t("kernel error:")} ${e instanceof Error ? e.message : String(e)}`,
      });
    } finally {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-4">
        <PaneTitlebarInset />
        {onBack && (
          <button className="text-text hover:opacity-60" aria-label={t("Back to notebooks")} onClick={onBack}>
        ...
        <span className="shrink-0 text-xs text-muted">{saved ? t("Saved") : t("Unsaved")}</span>
        <div className="flex-1" />
        <span className="hidden shrink-0 text-xs text-muted xl:inline">
          {t("Shift/⌘+Enter runs a cell · shared with the agent")}
        </span>
        <button
        ...
        <button
          ...
          aria-label={t("Reload from disk")}
          title={t("Reload (pick up the agent's changes)")}
          onClick={() => void load()}
        >
            ...
            <div key={cell.index} className="group mb-4">
              <div className="mb-1 flex items-center gap-2 text-xs text-muted">
                ...
                {isCodeLanguage(cell.language) &&
                  (running === cell.index ? (
                    ...
                    <button
                      ...
                      title={t("Stop — restarts this notebook's kernel (variables reset)")}
                      onClick={() => void stop()}
                    >
                    ...
                    <button
                      ...
                      aria-label={`${t("Run cell")} ${cell.index}`}
                ...
                <button
                  ...
                  aria-label={`${t("Delete cell")} ${cell.index}`}
