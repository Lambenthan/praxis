  ...
  const evaluate = async () => {
    ...
    try {
      ...
      if (res) setOutput(formatExecResult(res));
        ...
        onEvaluate(code);
        setOutput(t("→ sent to the agent's kernel"));
      } else {
        setOutput(t("(local kernel available only in the desktop app)"));
      }
    } catch (e) {
      setOutput(`${t("kernel error:")} ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="flex h-full flex-col">
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-4">
        ...
        <span className="text-sm font-medium text-text">{t("Notebook")}</span>
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-2">
        ...
        <span className="text-sm text-muted">{t("Shared with the agent")}</span>
        <div className="flex-1" />
        {data.live && (
          <span className="flex items-center gap-1 text-sm text-ok">
            <span className="h-1.5 w-1.5 rounded-full bg-ok" /> {t("Live")}
            <ChevronDown size={14} />
          </span>
        )}
      </div>
