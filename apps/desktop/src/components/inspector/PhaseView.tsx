    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="flex max-w-sm items-start gap-2 rounded-card border border-border bg-surface p-4 text-sm text-muted">
          ...
          <span>{t("Could not read this phase diagram —")} {parsed.error ?? t("unknown format")}.</span>
        </div>
      </div>
    );
  }
          ...
          (() => {
            ...
            return (
              <span>
                ...
                {e.stable ? t("stable (on hull)") : `+${e.eAboveHull.toFixed(3)} eV/atom ${t("above hull")}`}
              </span>
            );
          })()
        ) : (
          <span className="text-muted/50">{t("hover a phase · filled = stable, open = metastable")}</span>
        )}
      </div>
    </div>
  );
