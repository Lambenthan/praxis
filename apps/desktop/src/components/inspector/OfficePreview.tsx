  ...
  if (loading)
    return (
      <div className="flex items-center gap-2 p-4 text-sm text-muted">
        <Loader2 size={15} className="animate-spin" /> {t("Rendering…")}
      </div>
    );
  return null;
}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(`${t("Could not render this document:")} ${message(e)}`);
      } finally {
...
export function XlsxView({ bytes, scrollKey }: { bytes: ArrayBuffer; scrollKey: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(`${t("Could not read this workbook:")} ${message(e)}`);
      }
    })();
    return () => {
      cancelled = true;
  ...
  if (sheets.length === 0) return <div className="p-4 text-sm text-muted">{t("This workbook has no sheets.")}</div>;
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-border px-4 py-1.5 text-xs text-muted">
        {sheet?.truncated ? `${t("Truncated preview")} · ` : ""}{t("Embedded charts are not rendered.")}
      </div>
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(`${t("Could not render this presentation:")} ${message(e)}`);
      } finally {
        if (!cancelled) setLoading(false);
      }
    })();
