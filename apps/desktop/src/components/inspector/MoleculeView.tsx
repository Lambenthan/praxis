...
export function MoleculeView({ filename, text }: { filename: string; text: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (!model) {
          setError(t("No chemical structures found in this file."));
          return;
        }
  ...
  if (!format) return <div className="p-4 text-sm text-muted">{t("Not a chemical structure file.")}</div>;
  ...
  return (
    <div
      ...
      onWheel={onWheel}
    >
      <div ref={containerRef} className="absolute inset-0" aria-label={`${filename} ${t("3D molecule viewer")}`} />
      ...
      <div className="pointer-events-none absolute bottom-3 right-3 rounded-input border border-border/70 bg-surface/90 px-3 py-1.5 text-xs text-muted shadow-card backdrop-blur">
        ...
        {atomCount !== null && <span className="ml-2">{atomCount} {t("atoms")}</span>}
      </div>
      ...
      {(rendering || error) && (
        <div className="pointer-events-none absolute bottom-3 left-3 max-w-[70%] rounded-input border border-border/70 bg-surface/95 px-3 py-1.5 text-xs text-muted shadow-card backdrop-blur">
          {rendering ? t("Rendering structure…") : error}
        </div>
      )}
    </div>
  );
