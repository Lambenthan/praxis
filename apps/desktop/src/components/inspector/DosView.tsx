    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="flex max-w-sm items-start gap-2 rounded-card border border-border bg-surface p-4 text-sm text-muted">
          ...
          <span>{t("Could not read this DOSCAR —")} {parsed.error ?? t("unknown format")}.</span>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col bg-surface">
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center p-4">
        ...
        >
          ...
          {dos.spin && (
            <>
              <text x={pad.l + 4} y={pad.t + 10} className="fill-muted font-mono text-[10px]">↑ {t("spin up")}</text>
              <text x={pad.l + 4} y={H - pad.b - 4} className="fill-muted font-mono text-[10px]">↓ {t("spin down")}</text>
            </>
          )}
          <text x={W / 2} y={H - 4} textAnchor="middle" className="fill-muted font-mono text-[10px]">
            {alignFermi ? "E − E_F (eV)" : `${t("Energy")} (eV)`}
          </text>
      ...
      <div className="border-t border-border px-3 py-2 text-right font-mono text-[12px] text-muted">
        {hover ? (
          ...
          <span className="text-muted/50">{t("Fermi at E_F · hover to read a point")}</span>
        )}
      </div>
    </div>
  );
