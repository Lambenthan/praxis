    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="flex max-w-sm items-start gap-2 rounded-card border border-border bg-surface p-4 text-sm text-muted">
          ...
          <span>{t("Could not read this FITS file —")} {parsed.error}.</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-3 border-t border-white/10 px-3 py-2">
        ...
        <div className="ml-auto min-h-[2.25rem] text-right font-mono text-[12px] text-white/70">
          {hover ? (
            <>
              <div>
                {t("pixel")} ({hover.x}, {hover.y}) = {hover.v.toPrecision(5)}
              </div>
              {world && <div className="text-white/45">{fmtCoord(world.lon, world.lat)}</div>}
            </>
          ) : (
            <span className="text-white/30">{t("hover for pixel value & sky coordinate")}</span>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="flex h-full flex-col bg-surface">
      ...
      <div className="flex min-h-0 flex-1 items-center justify-center p-4">
        ...
        >
          ...
          <text x={(W) / 2} y={H - 6} textAnchor="middle" className="fill-muted font-mono text-[10px]">
            {ctype1 ?? t("sample")} {dx !== 1 ? `(${x0.toPrecision(4)} + ${dx}·i)` : ""}
          </text>
      ...
      <div className="border-t border-border px-3 py-2 text-right font-mono text-[12px] text-muted">
        {hover ? (
          ...
          <span className="text-muted/50">{t("hover to read a sample")}</span>
        )}
      </div>
    </div>
  );
