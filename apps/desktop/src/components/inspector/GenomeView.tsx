...
export function GenomeView({ filename, text }: { filename: string; text: string }) {
  ...
  if (!format) return <div className="p-4 text-sm text-muted">{t("Not a genome annotation file.")}</div>;
  if (!data || data.features.length === 0 || !contig || !view)
    return <div className="p-4 text-sm text-muted">{t("No features found in this")} {format.toUpperCase()} {t("file.")}</div>;
    ...
    <div className="flex h-full flex-col bg-surface">
      <div className="flex flex-wrap items-center gap-2 border-b border-border px-3 py-2 text-xs">
        ...
        <span className="text-muted">
          {formatBp(view.start)}–{formatBp(view.end)} · {contig.count} {t("features")}
        </span>
        <div className="flex-1" />
        <div className="flex items-center gap-1">
          <IconBtn label={t("Zoom in")} onClick={() => zoomAt(0.6, width / 2)}>
            <Plus size={13} />
          </IconBtn>
          <IconBtn label={t("Zoom out")} onClick={() => zoomAt(1.66, width / 2)}>
            <Minus size={13} />
          </IconBtn>
          <IconBtn
            label={t("Reset view")}
