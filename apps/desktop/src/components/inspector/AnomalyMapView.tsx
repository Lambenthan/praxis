    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="flex max-w-sm items-start gap-2 rounded-card border border-border bg-surface p-4 text-sm text-muted">
          ...
          <span>{t("Could not read this anomaly grid —")} {parsed.error ?? t("unknown format")}.</span>
        </div>
      </div>
    );
  }
        ...
        <div className="ml-auto min-h-[1.25rem] text-right font-mono text-[11px] text-white/70">
          {hover ? (
            <>
              ...
              {Number.isFinite(hover.v) ? `${hover.v > 0 ? "+" : ""}${hover.v.toPrecision(3)}` : t("no data")}
            </>
          ) : (
            <span className="text-white/30">{t("hover to read a grid cell")}</span>
          )}
        </div>
      </div>
    </div>
