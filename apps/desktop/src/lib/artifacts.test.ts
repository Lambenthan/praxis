...
describe("extractArtifactRefs", () => {
  ...
  it("finds coding files (.qcode) so a script-generated one is clickable", () => {
    const md = "编码完成,已生成 `open_coding.qcode`,点开切到裁决即可。";
    expect(extractArtifactRefs(md)).toEqual(["open_coding.qcode"]);
  });
});
...
describe("turnDeliverable", () => {
  ...
  it("returns null when the last turn produced nothing presentable", () => {
    expect(turnDeliverable([user("hi"), agent("wrote the script analysis.do and run.py")])).toBeNull();
  });

  it("falls back to a figure when the turn produced no adjudication file", () => {
    const d = turnDeliverable([user("plot it"), agent("saved the plot to _fig/wage_hist.png")]);
    expect(d?.path).toBe("_fig/wage_hist.png");
  });

  it("falls back to a table when the turn produced only a csv", () => {
    const d = turnDeliverable([user("describe"), agent("descriptives are in stats/summary.csv")]);
    expect(d?.path).toBe("stats/summary.csv");
  });

  it("an adjudication file outranks figures and tables from the same turn", () => {
    const d = turnDeliverable([
      user("go"),
      agent("plot saved to fig1.png and summary.csv"),
      agent("models are in results.qreg"),
    ]);
    expect(d?.path).toBe("results.qreg");
    // …even when the figure is mentioned AFTER the .qreg
    const d2 = turnDeliverable([
      user("go"),
      agent("models are in results.qreg, and a diagnostic plot in resid.png"),
    ]);
    expect(d2?.path).toBe("results.qreg");
  });

  it("finds a .qreg written by a file tool in the last turn", () => {
    const d = turnDeliverable([user("run it"), qreg("2026/results.qreg")]);
    expect(d?.path).toBe("2026/results.qreg");
  });

  it("finds a .qreg only named in the agent's prose (script-written files)", () => {
    const d = turnDeliverable([user("run it"), agent("3 models are in `2026/results.qreg` — click to review.")]);
    expect(d?.path).toBe("2026/results.qreg");
    expect(d?.filename).toBe("results.qreg");
  });

  it("ignores deliverables from earlier turns", () => {
    const d = turnDeliverable([
      user("first"),
      qreg("old/results.qreg"),
      user("something else"),
      agent("no files this time"),
    ]);
    expect(d).toBeNull();
  });

  it("the newest mention in the turn wins", () => {
    const d = turnDeliverable([
      user("go"),
      qreg("a/draft.qreg"),
      agent("final table saved to b/final.qreg"),
    ]);
    expect(d?.path).toBe("b/final.qreg");
  });

  it("covers .qcode the same way", () => {
    const d = turnDeliverable([user("code it"), agent("candidates are in interviews/i1.qcode")]);
    expect(d?.path).toBe("interviews/i1.qcode");
  });
});
