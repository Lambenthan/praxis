...
describe("stale running locks and interrupt", () => {
  ...
  it("Esc during the pre-session setup cancels locally — no abort, no session", async () => {
    ...
    await useRuntimeStore.getState().interrupt();
    const s = useRuntimeStore.getState();
    expect(s.sending).toBe(false); // composer unstuck immediately
    expect(mocks.abortSession).not.toHaveBeenCalled(); // nothing server-side yet
    expect(s.threads[DRAFT_KEY].blocks.slice(-1)[0]).toEqual({
      kind: "status-line",
      text: "Interrupted",
      tone: "error",
    });

    release(); // the stale turn resumes — and must abandon quietly
    expect(await send).toBeNull();
    const after = useRuntimeStore.getState();
    expect(after.currentId).toBeNull(); // no session was created for it
  ...
  it("a cancelled send does not clobber a NEW send started right after", async () => {
    ...
    const fresh = useRuntimeStore.getState().sendPrompt("second"); // normal path
    await new Promise((r) => setTimeout(r, 0));
    release(); // stale turn wakes up mid-fresh-send…
    await stale;
    await fresh;
    const s = useRuntimeStore.getState();
