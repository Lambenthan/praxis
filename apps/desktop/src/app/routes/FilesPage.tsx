        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2">
          ...
          {entries && entries.length === 0 && !error && (
            <div className="p-2 text-sm text-muted">
              {isTauri ? t("This folder is empty.") : t("The file explorer is available in the desktop app.")}
            </div>
          )}
          {entries?.map((entry) => (
            <button
      ...
      <div className="min-h-0 flex-1">
        {selected ? (
          ...
          <div className="flex h-full items-center justify-center p-8 text-center text-sm text-muted">
            {t("Select a file to preview it here.")}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-4">
        ...
        <span className="text-xs text-muted">{t("this session's folder")}</span>
        <div className="flex-1" />
        {controls}
        <button className="text-text hover:opacity-60" aria-label={t("Close files")} onClick={onClose}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        ...
        {error && <div className="p-2 text-sm text-error">{error}</div>}
        {entries && entries.length === 0 && !error && (
          <div className="p-2 text-sm text-muted">{t("This folder is empty.")}</div>
        )}
        {entries?.map((entry) => (
          <button
            key={entry.path}
