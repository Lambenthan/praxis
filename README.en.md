...
The **[Releases](https://github.com/Lambenthan/fishes/releases/latest) page has ready-to-run packaged builds** — no environment setup, no compiling from source. Pick the file for your OS and run it.
...
_(Alternative — one command in Terminal: `xattr -cr /Applications/Fishes.app`, then double-click. Right-clicking "Open" no longer bypasses this on recent macOS. Intel Macs: use [`Fishes_0.3.0_x64.dmg`](https://github.com/Lambenthan/fishes/releases/download/v0.3.0/Fishes_0.3.0_x64.dmg); the same allow-once steps apply.)_

**Linux** — `.deb` and `.rpm` packages are on the Releases page.
